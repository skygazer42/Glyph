...
class HybridSqlGeneratorAgent(BaseAgent):
    ...
    async def handle_analysis_message(self, message: AnalysisMessage, ctx: MessageContext) -> None:
        ...
        try:
            ...
            if self._initialized and settings.system.hybrid_retrieval_enabled:
                try:
                    similar_examples = await self.hybrid_engine.hybrid_retrieve(
                        query=query,
                        schema_context=schema_context,
                        connection_id=connection_id or 0,
                        top_k=settings.system.max_examples_per_query
                    )
            ...
            # 学习新的问答对
            if self._initialized and settings.system.auto_learning_enabled and self._is_valid_sql(sql_content):
