...
class RewriteAgent:
    ...
    async def rewrite(self, query: str, context: dict | None = None, domain_hint: dict | None = None) -> str:
        ...
        prompt = (
            "你是一名政务/政策问答场景下的改写助手。\n"
            "请在严格保留用户【最后一问原意】的前提下，将问题改写成更清晰、专业、便于业务逻辑匹配的“自包含单句问题”。\n"
            "要求：\n"
            "1. 只能在不改变意图类型的前提下进行改写，比如：资格判断仍然是资格判断，金额计算仍然是金额计算，流程咨询仍然是流程咨询；禁止把“是否符合条件”改写成“这个政策的具体内容是什么”这类含义变化。\n"
            "2. 可以根据上下文补齐明显缺失但已给出的信息（如城市、政策名称、产品类别、金额、时间范围等），但不得臆造上下文中未出现的新事实。\n"
            "3. 若检测到领域关键词，需在改写后的句子中以规范词形呈现；无法确认的字段保持原词或不写。\n"
            "4. 对话历史和上下文摘要仅作为理解背景的参考，禁止把历史回答内容当成新的提问内容合并进去。\n"
            "5. 只输出改写后的单句问题，不要输出任何解释或前后缀说明。\n\n"
            f"领域提示：{domain_hints}\n"
            f"上下文摘要：\n{summary}\n"
            f"原始问题：{normalized}"
        )
