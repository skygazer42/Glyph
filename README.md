...
> 仓库已内置后端 `Dockerfile` 与前端多阶段镜像 `web/Dockerfile`，配合 `docker-compose.yaml` 可一键起服务。

#### 一键启动（前后端 + 依赖）
```bash
cp .env.example .env   # 填好 LLM/Embedding 等密钥
docker compose up -d --build web   # 会同时拉起 api + mysql + milvus + redis + 前端
```
