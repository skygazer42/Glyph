...
路由图展示了 FAQ 短路、FastPath 分流与各专业 Agent 的执行关系。

<div align="center">
  <img src="docs/images/glyph-routing-flow.png" alt="Glyph 查询处理与路由执行流程图" width="90%" />
</div>
...
推荐直接使用仓库根目录的 `docker-compose.yaml` 进行部署。
...
```bash
# 备份 MySQL
mysqldump -u root -p policy_db > backup.sql
